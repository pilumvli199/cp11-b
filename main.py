#!/usr/bin/env python3
# Advanced Crypto Trading Bot - Complete Signal Management System (Final patched)
# - 720 candles analysis (4H + 1H)
# - Redis tracking with JSON serialization for complex fields
# - Concurrency control via asyncio.Semaphore for HTTP requests
# - Safer OpenAI call (no unsupported kwargs) using run_in_executor
# - Progress alerts, trend reversal checks, and signal lifecycle management
...
# ---------------- CONFIG ----------------
SYMBOLS = os.getenv("SYMBOLS", "BTCUSDT,ETHUSDT,SOLUSDT,XRPUSDT,AAVEUSDT,TRXUSDT,DOGEUSDT,BNBUSDT,ADAUSDT,LTCUSDT,LINKUSDT").split(",")
...
def save_signal_to_redis(symbol, signal_data):
    ...
    try:
        ...
        store = dict(signal_data)  # shallow copy
